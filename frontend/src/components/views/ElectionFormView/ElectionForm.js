    ...
    if (name === 'candidatesEmpty') {
      if (this.state.candidates.length === 0) {
        ...
        for (let i = 0; i < this.state.candidates.length; i++) {
          if (this.state.candidates[i].name === '') {
        ...
        <Grid container justify="flex-end" spacing={3}>
          ...
          <Grid item xs={12}>
            <Alert variant="filled" severity="warning">
              Once you create an election, you must deploy it in the Upcoming
              Elections tab, for that election to be available for voting
            </Alert>
          </Grid>
          <Grid item>
            <Button
