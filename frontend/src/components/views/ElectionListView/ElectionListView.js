...
// Parse election date to produce a duration string
function getDurationString(election) {
  ...
  // Upcoming election
  if (now < start) {
    return `Starts in ${distanceInWordsToNow(start)}`;
  }
  // Previous election
  else if (now > end) {
    return `Ended ${distanceInWordsToNow(end)} ago`;
  }
  // In progress election
  else {
    return `Ends in ${distanceInWordsToNow(end)}`;
  }
}
    ...
    axios({
      ...
      .catch((error) => {
        ...
        if (error.response.status === 400) {
          alert('The url ' + URL + `/api/election/${props.id}/deploy`);
          alert(
            `An error occured while deploying election ${props.election.electionName}. Make sure you have validated at least 1 voter and try again.`
          );
        }
        if (error.response.status === 401) {
          window.sessionStorage.clear();
