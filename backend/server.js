...
const env = '/usr/lib/node';
const mnemonic =
  'prosper whale grocery gift trial corn build valid clap razor truth ginger';
const URL = 'http://127.0.0.1:7545';
const databaseURL = 'https://blockvote-f14c0-default-rtdb.firebaseio.com';
...
const validateVoters = async (
  ...
  for (let id in validVoters) {
    ...
    console.log('validVoterAddresses ' + validVoterAddresses);
    voterProvider.engine.stop();
  }
  console.log('VALID VOTER ADDRESSES ', validVoterAddresses);
  ...
  console.log('invalidVoterIDs ' + invalidVoterIDs);
...
app.post('/api/election/create', verifyUser, async (req, res) => {
  ...
  const { candidates, startTime, endTime, validVoters, electionName, userID } =
    req.body;
  try {
...
app.put('/api/election/:electionID/deploy', verifyUser, async (req, res) => {
  ...
  try {
    ...
    console.log('allElectionData ' + allElectionData);
    let electionData = {};
    ...
    console.log('Despues de desplegar ');
    // Get reference to the specific election in the database
    const electionRef = db.ref('elections/' + electionID);
    ...
    console.log('ELECTIONREF ' + electionRef);
    console.log('validVoters ' + validVoter);
  } catch (error) {
    console.log('ERROR ' + error);
    console.log('invalidVoterIDs');
    res.status(400).send('bad request');
  }
});
...
app.put('/api/election/:electionID/update', verifyUser, async (req, res) => {
  ...
  const { electionName, candidates, startTime, endTime, validVoters } =
    req.body;
  try {
...
app.get('/api/election/:electionID/result', verifyUser, async (req, res) => {
  ...
  try {
    ...
    if (Account === null || electionDetails === null) {
      console.log(
        'Error 400. La cuenta es nula o los datos de la eleccion no son correctos'
      );
      res.status(400).send('bad request');
      return;
    }
    ...
    let timestamp = contract.events.timestamp();
    console.log(timestamp);
    timestamp.callback(console.log);
    const numOfCandidates = await contract.methods.numberOfCandidates().call();

    console.log('Numero de Candidatos ' + numOfCandidates);
    let tempResults = [];
    let numVotes = 0;
